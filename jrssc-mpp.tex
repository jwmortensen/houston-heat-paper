...
\begin{algorithm}
  ...
  \BlankLine
  \For{$p$ in $1:P$}{
  Sample $N$ draws from $\delta_{p} \sim \mathcal{G}(n_p + 0.001, 1.001)$\;
  Sample $N$ draws from $\boldsymbol\alpha_p \sim \text{Dir}\left(n_{p,0}+1, n_{p,1}+1, \dots, n_{p,113} + 1\right)$\;
  Sample $N$ draws from $\boldsymbol\gamma_p \sim \text{Dir}\left(n_{p,\text{Female}} + 1, n_{p,\text{Male}} + 1\right)$\;
  Sample $N$ draws from $\boldsymbol\xi_p \sim \text{Dir}\left(n_{p,\text{Asian}} + 1, \dots, n_{p,\text{White}}\right)$\;
  }
  \For{$i$ in $1:N$}{
    \For{$p$ in $1:P$} {
      Sample $\sigma^2_p \sim \left[ \sigma^2_p | \boldsymbol{\phi}^\star_p, \mathbf{M}_p\right]$\label{draws2}\;
      Sample $\boldsymbol\phi^\star_p \sim \left[ \boldsymbol\phi^\star_p | \sigma^2_p, \mathbf{M}_p \right]$\label{drawphi}\;
    }
    Sample $\sigma^2_\mu \sim \left[ \sigma^2_\mu | \boldsymbol{\mu}, \boldsymbol{\beta}, \mathbf{M}_\mu \right]$\label{drawsmu}\;
    Sample $\boldsymbol{\mu} \sim \left[ \boldsymbol{\mu} | \{\boldsymbol{\phi}^\star_p\}, 
    \{\sigma^2_p\}, \{\mathbf{M}_p\}, \sigma^2_\mu, \mathbf{M}_\mu, \boldsymbol{\beta} \right]$\label{drawmu}\;
    Sample $\boldsymbol{\beta} \sim \left[ \boldsymbol{\beta} | \boldsymbol{\mu}, \sigma^2_\mu, \mathbf{M}_\mu \right]$\label{drawbeta}\;
  }
...
\begin{table}
  ...
  Model 02 & X & X &   &   &   &   &   &   &   & 12345 \\
  Model 03 &   &   & X & X &   &   &   &   &   & 220523 \\
  Model 04 &   &   &   &   & X & X & X & X &   & 220482 \\
  Model 05 & X & X & X & X &   &   &   &   &   & 220526 \\
  Model 06 & X & X &   &   & X & X & X & X &   & 220512 \\
  Model 07 &   &   & X & X & X & X & X & X &   & 220497 \\
  Model 08 & X & X & X & X & X & X & X & X &   & 220498 \\
  Model 09 & X & X &   &   &   &   &   &   & X & 220529 \\
  Model 10 &   &   & X & X &   &   &   &   & X & 220455 \\
  Model 11 &   &   &   &   & X & X & X & X & X & 220493 \\
  Model 12 & X & X & X & X &   &   &   &   & X & 220522 \\
